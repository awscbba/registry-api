...
async def test_email_validation_fix():
    ...
    try:
        ...
        assert (
            person.email == "noreply@people-register.com"
        ), f"Expected converted email, got: {person.email}"
...
async def test_other_email_cases():
    ...
    test_cases = [
        {"name": "Empty email", "email": "", "expected": "unknown@example.com"},
        {
            "name": "Missing @ symbol",
            "email": "invalid-email",
    ...
    for case in test_cases:
        ...
        try:
            ...
            if person.email == case["expected"]:
                ...
                print(
                    f"❌ {case['name']}: Expected '{case['expected']}', got '{person.email}'"
                )
                return False
        except Exception as e:
