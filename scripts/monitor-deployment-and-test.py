...
def test_person_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Person: {person_data.get('data', {}).get('firstName', 'Unknown')} {person_data.get('data', {}).get('lastName', 'Unknown')}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Updated: {updated_data.get('data', {}).get('firstName', 'Unknown')} {updated_data.get('data', {}).get('lastName', 'Unknown')}")
        else:
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test-only":
        ...
        try:
            ...
            print("You can test manually with: python scripts/monitor-deployment-and-test.py --test-only")
            sys.exit(0)
