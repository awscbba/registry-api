...
def fix_cascade_deletion():
    ...
    try:
        ...
        while 'LastEvaluatedKey' in people_response:
            people_response = people_table.scan(ExclusiveStartKey=people_response['LastEvaluatedKey'])
        ...
        while 'LastEvaluatedKey' in subscriptions_response:
            subscriptions_response = subscriptions_table.scan(ExclusiveStartKey=subscriptions_response['LastEvaluatedKey'])
        ...
        if orphaned_subscriptions:
            ...
            print(f"")
            print(f"🤔 Do you want to delete these {len(orphaned_subscriptions)} orphaned subscriptions?")
            print(f"   This will clean up the data inconsistency and fix your smart card count.")
            ...
            if response.lower() == 'yes':
                ...
                for subscription in orphaned_subscriptions:
                    try:
                        ...
                        if subscription_id:
                            ...
                            cleaned_count += 1
                            print(f"   ✅ Deleted subscription {subscription_id} (person: {person_id})")
                        else:
                            failed_count += 1
                        ...
                        failed_count += 1
                        print(f"   ❌ Error deleting subscription {subscription.get('id', 'unknown')}: {e}")
                ...
                print(f"")
                print(f"📊 Your smart card should now show {len(all_subscriptions) - cleaned_count} subscriptions")
            else:
...
def print_code_fix():
    ...
    print("=" * 60)
    print("Replace the delete_person method in src/services/defensive_dynamodb_service.py:")
    print("")
    ...
    print("        person_subscriptions = await self.get_subscriptions_by_person(person_id)")
    print("        ")
    ...
    print("                try:")
    print("                    success = await self.delete_subscription(subscription_id)")
    print("                    if success:")
    print("                        deleted_subscriptions += 1")
    print("                        self.logger.info(f'Deleted subscription {subscription_id} for person {person_id}')")
    print("                except Exception as e:")
    print("                    self.logger.error(f'Error deleting subscription {subscription_id}: {e}')")
    print("        ")
    print("        self.logger.info(f'Deleted {deleted_subscriptions} subscriptions for person {person_id}')")
    print("        ")
    ...
    print("=" * 60)
    print("1. Create feature branch: git checkout -b fix/cascade-deletion-subscriptions")
