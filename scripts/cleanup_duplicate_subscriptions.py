...
async def resolve_duplicate_subscriptions(duplicates, dry_run=True):
    ...
    for key, subscriptions in duplicates.items():
        ...
        keep_subscription = (
            active_subscription or pending_subscription or sorted_subscriptions[0]
        )

        print(
            f"   ✅ Keeping: {keep_subscription['id'][:8]}... (status: {keep_subscription.get('status')})"
        )
        ...
        for sub in sorted_subscriptions:
            if sub["id"] != keep_subscription["id"]:
                print(
                    f"   🗑️  Marking as inactive: {sub['id'][:8]}... (was: {sub.get('status')})"
                )
...
async def main():
    ...
    for key, subscriptions in duplicates.items():
        person_id, project_id = key.split(":", 1)
        print(
            f"   👤 Person {person_id[:8]}... 📁 Project {project_id[:8]}... → {len(subscriptions)} duplicates"
        )

        for sub in subscriptions:
            print(
                f"      - {sub['id'][:8]}... ({sub.get('status')}) created: {sub.get('createdAt', 'unknown')}"
            )
