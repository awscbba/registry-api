...
async def test_field_mapping_consistency():
    ...
    try:
        ...
        model_data_by_alias = person_update.model_dump(by_alias=True, exclude_unset=True)
        print(f"📋 Model data (by_alias=True): {json.dumps(model_data_by_alias, indent=2)}")
        
        # Check model_dump without by_alias (internal field names)
        model_data_internal = person_update.model_dump(exclude_unset=True)
        print(f"🔧 Model data (internal): {json.dumps(model_data_internal, default=str, indent=2)}")
    ...
    for field, value in update_data.items():
        ...
        if field in ["first_name", "last_name", "email", "phone", "date_of_birth", 
                    "address", "is_admin", "is_active", "failed_login_attempts", 
                    "account_locked_until", "require_password_change"]:
    ...
    try:
        ...
        if camel_internal == snake_internal:
            ...
            print("   📝 This is expected - PersonUpdate model uses aliases for camelCase")
            print("   📝 Frontend should send camelCase, backend processes as snake_case")
