...
class TestVersionedAPIHandlerSource:
    ...
    def test_has_required_imports(self, handler_source):
        ...
        for import_stmt in required_imports:
            assert import_stmt in handler_source, f"Missing required import: {import_stmt}"
    ...
    def test_has_health_endpoint(self, handler_source):
        ...
        assert "async def health_check" in handler_source, "Health check function not found"
    ...
    def test_async_functions_properly_defined(self, handler_source):
        ...
        assert len(non_async_endpoints) == 0, f"Non-async endpoint functions: {non_async_endpoints}"
        assert len(async_functions) > 5, f"Expected more async functions, found: {len(async_functions)}"
        ...
        for method in async_db_methods:
            ...
            if pattern in handler_source:
                ...
                for match in matches:
                    ...
                    line = handler_source[start:end] if end != -1 else handler_source[start:]
    ...
    def test_uses_standardized_components(self, handler_source):
        ...
        for component in standardized_components:
            assert component in handler_source, f"Missing standardized component: {component}"
    ...
    def test_has_proper_error_handling(self, handler_source):
        ...
        assert "handle_database_error" in handler_source, "Not using standardized database error handling"
    ...
    def test_version_information_in_responses(self, handler_source):
        ...
        found_version_info = any(pattern in handler_source for pattern in v2_version_patterns)
        assert found_version_info, "V2 endpoints should include version information in responses"
