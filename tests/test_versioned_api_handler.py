...
class TestVersionedAPIHandler:
    ...
    def mock_db_service(self):
        ...
        with patch('handlers.versioned_api_handler.db_service') as mock:
            ...
            mock.get_all_subscriptions = AsyncMock(return_value=[
                {"id": "sub1", "projectId": "proj1", "personId": "person1", "status": "active"}
            ])
            mock.get_all_projects = AsyncMock(return_value=[
                {"id": "proj1", "name": "Test Project", "description": "Test"}
            ])
            mock.get_project_by_id = MagicMock(return_value={
            ...
            mock.create_subscription = AsyncMock(return_value={
                "id": "sub1", "projectId": "proj1", "personId": "person1"
            })
            ...
            mock.update_person = AsyncMock(return_value=MagicMock(
                id="person1", email="test@example.com", first_name="Test",
    ...
    def test_all_routes_registered(self, client):
        ...
        for expected_route in expected_routes:
            assert any(expected_route.replace("{person_id}", "person_id") in route or 
                      expected_route in route for route in routes), f"Route {expected_route} not found"
        
        # Verify we have the expected number of routes (approximately)
        assert len(routes) >= len(expected_routes), f"Expected at least {len(expected_routes)} routes, got {len(routes)}"
    ...
    def test_no_duplicate_routes(self, client):
        ...
        assert len(routes) == len(unique_routes), f"Duplicate routes found: {[r for r in routes if routes.count(r) > 1]}"
...
class TestIntegration:
    ...
    def test_full_subscription_flow_v2(self, client):
        ...
        with patch('handlers.versioned_api_handler.db_service') as mock_db:
            ...
            mock_db.get_project_by_id.return_value = {"id": "proj1", "name": "Test Project"}
            ...
            mock_db.create_subscription.return_value = {
                "id": "sub1", "projectId": "proj1", "personId": "person1", "status": "pending"
            }
