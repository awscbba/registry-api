...
class TestSubscriptionCountFix:
    ...
    def mock_subscriptions(self):
        ...
        return [
            ...
            {"id": "sub-1", "projectId": "project-1", "personId": "person-1", "status": "active"},
            {"id": "sub-2", "projectId": "project-1", "personId": "person-2", "status": "active"},
            {"id": "sub-3", "projectId": "project-1", "personId": "person-3", "status": "pending"},
            {"id": "sub-4", "projectId": "project-1", "personId": "person-4", "status": "inactive"},  # Should be excluded
            {"id": "sub-5", "projectId": "project-1", "personId": "person-5", "status": "inactive"},  # Should be excluded
            
            # Project 2 subscriptions
            {"id": "sub-6", "projectId": "project-2", "personId": "person-6", "status": "active"},
            {"id": "sub-7", "projectId": "project-2", "personId": "person-7", "status": "pending"},
            {"id": "sub-8", "projectId": "project-2", "personId": "person-8", "status": "inactive"},  # Should be excluded
        ]

    @pytest.mark.asyncio
    async def test_admin_projects_subscription_count_excludes_inactive(self, mock_projects, mock_subscriptions):
        ...
        with patch('src.handlers.versioned_api_handler.db_service') as mock_db:
            ...
            with patch('src.handlers.versioned_api_handler.create_v2_response') as mock_response:
                with patch('src.handlers.versioned_api_handler.logger'):
                    ...
                    call_args = mock_response.call_args[0][0]  # First positional argument
                    
                    # Verify project 1 counts
                    project_1 = next(p for p in call_args if p["id"] == "project-1")
                    assert project_1["subscriptionCount"] == 3  # 2 active + 1 pending (excludes 2 inactive)
                    assert project_1["activeSubscriptions"] == 2
                    assert project_1["pendingSubscriptions"] == 1
                    assert project_1["totalSubscriptionsEverCreated"] == 5  # All including inactive
                    ...
                    assert project_2["subscriptionCount"] == 2  # 1 active + 1 pending (excludes 1 inactive)
                    assert project_2["activeSubscriptions"] == 1
                    assert project_2["pendingSubscriptions"] == 1
                    assert project_2["totalSubscriptionsEverCreated"] == 3  # All including inactive
                    assert project_2["availableSlots"] == 4  # 5 max - 1 active

    @pytest.mark.asyncio
    async def test_admin_dashboard_subscription_count_excludes_inactive(self, mock_projects, mock_subscriptions):
        ...
        with patch('src.handlers.versioned_api_handler.db_service') as mock_db:
            ...
            with patch('src.handlers.versioned_api_handler.create_v2_response') as mock_response:
                with patch('src.handlers.versioned_api_handler.logger'):
                    ...
                    # Get the data passed to create_v2_response
                    dashboard_data = mock_response.call_args[0][0]  # First positional argument
                    
                    # Verify dashboard counts exclude inactive subscriptions
                    assert dashboard_data["totalSubscriptions"] == 5  # 3 active + 2 pending (excludes 3 inactive)
                    assert dashboard_data["activeSubscriptions"] == 3
                    assert dashboard_data["pendingSubscriptions"] == 2
                    assert dashboard_data["totalSubscriptionsEverCreated"] == 8  # All including inactive
                    
                    # Verify statistics also exclude inactive
                    stats = dashboard_data["statistics"]
                    assert stats["averageSubscriptionsPerProject"] == 2.5  # 5 current subscriptions / 2 projects
    ...
    async def test_subscription_count_after_deactivation_scenario(self):
        ...
        initial_subscriptions = [
            {"id": "sub-1", "projectId": "project-1", "personId": "person-1", "status": "active"},
            {"id": "sub-2", "projectId": "project-1", "personId": "person-2", "status": "active"},
            {"id": "sub-3", "projectId": "project-1", "personId": "person-3", "status": "active"},
        ]
        ...
        after_deactivation_subscriptions = [
            {"id": "sub-1", "projectId": "project-1", "personId": "person-1", "status": "active"},
            {"id": "sub-2", "projectId": "project-1", "personId": "person-2", "status": "active"},
            {"id": "sub-3", "projectId": "project-1", "personId": "person-3", "status": "inactive"},  # Deactivated
        ]
        ...
        with patch('src.handlers.versioned_api_handler.db_service') as mock_db:
            ...
            mock_db.get_all_subscriptions = AsyncMock(return_value=initial_subscriptions)
            ...
            mock_db.get_all_subscriptions = AsyncMock(return_value=after_deactivation_subscriptions)
            ...
            with patch('src.handlers.versioned_api_handler.create_v2_response') as mock_response:
                with patch('src.handlers.versioned_api_handler.logger'):
                    ...
                    assert after_data["subscriptionCount"] == 2  # Should decrease from 3 to 2
                    assert after_data["activeSubscriptions"] == 2
                    assert after_data["totalSubscriptionsEverCreated"] == 3  # Historical count unchanged
