...
async def test_all_update_inconsistencies():
    ...
    try:
        ...
        for field, value in person_internal.items():
            if field == "account_locked_until" and isinstance(value, str):
                print(f"   ⚠️ Field '{field}' is string, needs type checking in DB service")
    ...
    try:
        ...
        for field, value in project_internal.items():
            if field in ["startDate", "endDate"] and isinstance(value, str):
                print(f"   ✅ Field '{field}' is string date - should be handled correctly")
            elif field == "status" and hasattr(value, 'value'):
                print(f"   ⚠️ Field '{field}' is enum object, DB service calls .value - check if this works")
            elif field == "status" and isinstance(value, str):
                print(f"   ⚠️ Field '{field}' is string, but DB service expects enum with .value")
                issues_found.append(f"ProjectUpdate.status: String vs Enum inconsistency")
    ...
    try:
        ...
        for field, value in subscription_internal.items():
            if field == "status" and hasattr(value, 'value'):
                ...
                print(f"   ⚠️ Field '{field}' is string, but DB service expects enum with .value")
                issues_found.append(f"SubscriptionUpdate.status: String vs Enum inconsistency")
    ...
    try:
        ...
        if ".value" in service_code:
            ...
            if "status.value" in service_code:
                print("   🔍 Found status.value calls - verify enum objects are passed correctly")
                issues_found.append("Enum handling: .value calls found, need to verify enum objects")
    ...
    project_fields = ["name", "description", "startDate", "endDate", "maxParticipants", "status"]
