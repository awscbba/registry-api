...
class ProductionRiskFixer:
                ...
                if (
                    ...
                    for i in range(context_start, context_end):
                        if "update_item(" in lines[i]:
                            ...
                            self.apply_conditional_parameter_fix(
                                file_path, lines, i, line_idx
                            )
                            break

    def apply_conditional_parameter_fix(
        self,
        file_path: Path,
        lines: List[str],
        update_item_line: int,
        expression_names_line: int,
    ):
    ...
    def generate_fix_report(self) -> str:
        ...
        for fix_type, fixes in fixes_by_type.items():
            ...
            for fix in fixes:
                report.append(
                    f"### {fix['file']}:{fix.get('line', fix.get('lines', ''))}"
                )
