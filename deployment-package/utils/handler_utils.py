...
async def log_security_event_with_context(
    ...
    return await logging_service.log_security_event(
        event_type=event_type, context=context, details=details
    )


async def check_rate_limit_for_endpoint(
    limit_type: RateLimitType, request: Request, user_id: Optional[str] = None
):
    ...
    if user_id and limit_type in [
        RateLimitType.PASSWORD_CHANGE,
        RateLimitType.PERSON_UPDATES,
    ]:
        identifier = user_id

    # Check rate limit
    result = await rate_limiting_service.check_rate_limit(
        limit_type, identifier, context
    )
...
def create_validation_exception_from_errors(
    field_errors: Dict[str, str], request: Request, user_id: Optional[str] = None
) -> APIException:
...
def create_authentication_exception(
    message: str = "Authentication failed", request: Optional[Request] = None
) -> APIException:
...
def create_password_policy_exception(
    message: str, request: Request, user_id: Optional[str] = None
) -> APIException:
    """Create password policy violation exception."""
    context = create_error_context(request, user_id)
    return APIException(
        error_code=ErrorCode.PASSWORD_POLICY_VIOLATION, message=message, context=context
    )


def create_email_already_exists_exception(
    email: str, request: Request, user_id: Optional[str] = None
) -> APIException:
...
async def handle_service_error(
    error: Exception, operation: str, request: Request, user_id: Optional[str] = None
) -> APIException:
    ...
    if "not found" in str(error).lower():
        return APIException(
            error_code=ErrorCode.RESOURCE_NOT_FOUND, message=str(error), context=context
        )
    elif "already exists" in str(error).lower() or "duplicate" in str(error).lower():
        return APIException(
            error_code=ErrorCode.DUPLICATE_VALUE, message=str(error), context=context
        )
    elif "validation" in str(error).lower() or "invalid" in str(error).lower():
        return APIException(
            error_code=ErrorCode.INVALID_FORMAT, message=str(error), context=context
        )
    else:
        return APIException(
...
async def log_person_list_access(
    request: Request, user_id: str, count: int, limit: int, success: bool = True
):
...
async def log_person_access(
    person_id: str, request: Request, user_id: str, success: bool = True
):
