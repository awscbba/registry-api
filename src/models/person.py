...
class Person(PersonBase):
    ...
    password_hash: Optional[str] = Field(None, exclude=True)  # Never include in API responses
    password_salt: Optional[str] = Field(None, exclude=True)  # Never include in API responses
    require_password_change: bool = False
    last_password_change: Optional[datetime] = Field(None, alias="lastPasswordChange")
    password_history: List[str] = Field(default_factory=list, exclude=True)  # Never include in API responses
    ...
    email_verification_token: Optional[str] = Field(None, exclude=True)  # Never include in API responses
...
class PersonAdminResponse(BaseModel):
    ...
    def from_person(cls, person: Person) -> "PersonAdminResponse":
        ...
        return cls(
            ...
            accountLockedUntil=person.account_locked_until.isoformat() if person.account_locked_until else None,
            ...
            lastPasswordChange=person.last_password_change.isoformat() if person.last_password_change else None
        )
...
class PersonSearchRequest(BaseModel):
    ...
    email_verified: Optional[bool] = Field(None, alias="emailVerified", description="Filter by email verification status")
...
class PersonSearchResponse(BaseModel):
    ...
    @classmethod
    def create(cls, people: List[Person], total_count: int, limit: int, offset: int) -> "PersonSearchResponse":
...
class AdminUnlockRequest(BaseModel):
    ...
    reason: str = Field(..., min_length=10, description="Reason for unlocking the account")
...
class PersonDeletionRequest(BaseModel):
    ...
    confirmation_token: str = Field(..., description="Confirmation token from initial deletion request")
...
class PersonDeletionResponse(BaseModel):
    ...
    success: bool
    message: str
    confirmation_token: Optional[str] = Field(None, description="Token required for final confirmation")
    expires_at: Optional[datetime] = Field(None, description="When the confirmation token expires")
    subscriptions_found: Optional[int] = Field(None, description="Number of active subscriptions found")
...
class ReferentialIntegrityError(BaseModel):
    ...
    message: str
    constraint_type: str = Field(description="Type of constraint violated (e.g., 'subscriptions')")
    related_records: List[Dict[str, Any]] = Field(description="Details of related records preventing deletion")
