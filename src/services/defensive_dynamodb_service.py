...
class DefensiveDynamoDBService:
    ...
    async def create_person(
        ...
        with create_subsegment("create_person") as subsegment:
            try:
                ...
                add_metadata(
                    "dynamodb", "error_message", e.response["Error"]["Message"]
                )
    ...
    async def update_person(
        ...
        try:
            ...
            # Manually add password fields (they are excluded from model_dump due to exclude=True)
            if (
                hasattr(person_update, "password_hash")
                and person_update.password_hash is not None
            ):
                update_data["password_hash"] = person_update.password_hash
            if (
                hasattr(person_update, "password_salt")
                and person_update.password_salt is not None
            ):
        ...
        with create_subsegment("get_person") as subsegment:
            try:
                ...
                if "Item" in response:
                    ...
                    add_metadata(
                        "dynamodb", "email", person.email if person else "unknown"
                    )
                    return person
                else:
                ...
                raise Exception(
                    f"Failed to get person: {e.response['Error']['Message']}"
                )
            except Exception as e:
                add_annotation("error", "general_error")
                add_metadata("dynamodb", "error", str(e))
