...
class DynamoDBService:
    def __init__(self):
        ...
        self.lockout_table_name = os.environ.get('LOCKOUT_TABLE_NAME', 'AccountLockoutTable')

        # New tables for projects and subscriptions
        self.projects_table_name = os.environ.get('PROJECTS_TABLE_NAME', 'ProjectsTable')
        self.subscriptions_table_name = os.environ.get('SUBSCRIPTIONS_TABLE_NAME', 'SubscriptionsTable')
    ...
    def _check_email_gsi(self):
        ...
        try:
            ...
            if not gsi_exists:
                self.logger.warning("EmailIndex GSI not found on people table. Email uniqueness checks will use scan operations.")
        ...
        try:
            ...
            if context:
                audit_entry.update({
                    ...
                    'ip_address': context.ip_address if context.ip_address else None,
                    'user_agent': context.user_agent if context.user_agent else None,
            ...
            if before_state and after_state and 'changed_fields' in audit_entry:
                log_message += f" - Changed fields: {', '.join(audit_entry['changed_fields'])}"
    ...
    def _handle_database_error(self, operation: str, error: ClientError, context: Optional[ErrorContext] = None) -> Exception:
        """Enhanced error handling for database constraint violations with detailed error mapping"""
        error_code = error.response['Error']['Code']
        error_message = error.response['Error']['Message']

        # Log the error
        self.logger.error(f"Database error in {operation}: {error_code} - {error_message}")
    ...
    async def check_email_uniqueness(self, email: str, exclude_person_id: Optional[str] = None, context: Optional[ErrorContext] = None) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                if gsi_error.response['Error']['Code'] == 'ValidationException' and 'IndexName' in gsi_error.response['Error']['Message']:
                    self.logger.warning("EmailIndex GSI not found, falling back to scan for email uniqueness check")
                    ...
                    await self._log_database_operation(
                        ...
                        context=context,
                        success=True,
                        additional_data={"using_gsi": False, "fallback_reason": "GSI not available"}
                    )
    ...
    def _person_to_item(self, person: Person) -> Dict[str, Any]:
        ...
        if hasattr(person, 'email_verification_token') and person.email_verification_token:
    ...
    def _item_to_person(self, item: Dict[str, Any]) -> Person:
        ...
        if 'lastPasswordChange' in item:
            person.last_password_change = datetime.fromisoformat(item['lastPasswordChange'])
        ...
        if 'accountLockedUntil' in item:
            person.account_locked_until = datetime.fromisoformat(item['accountLockedUntil'])
    ...
    async def create_person(self, person_data: PersonCreate, context: Optional[ErrorContext] = None) -> Person:
    ...
    async def get_person(self, person_id: str, context: Optional[ErrorContext] = None) -> Optional[Person]:
    ...
    async def list_people(self, limit: int = 100, context: Optional[ErrorContext] = None) -> List[Person]:
    ...
    async def update_person(self, person_id: str, person_update: PersonUpdate, context: Optional[ErrorContext] = None) -> Optional[Person]:
    ...
    async def update_person_password_fields(
        ...
        if last_password_change is not None:
            ...
            expression_attribute_values[':last_password_change'] = last_password_change.isoformat()

        if require_password_change is not None:
            update_expression += ", requirePasswordChange = :require_password_change"
            expression_attribute_values[':require_password_change'] = require_password_change

        if failed_login_attempts is not None:
            update_expression += ", failedLoginAttempts = :failed_login_attempts"
            expression_attribute_values[':failed_login_attempts'] = failed_login_attempts

        if account_locked_until is not None:
            update_expression += ", accountLockedUntil = :account_locked_until"
            expression_attribute_values[':account_locked_until'] = account_locked_until.isoformat()
        ...
        try:
            ...
            raise self._handle_database_error("update_person_password_fields", e, context)
    ...
    async def delete_person(self, person_id: str, context: Optional[ErrorContext] = None) -> bool:
        ...
        try:
            ...
            if subscriptions:
                ...
                raise ValueError(f"Cannot delete person with {len(subscriptions)} active subscriptions. Please remove subscriptions first.")
        except Exception as e:
            if "Cannot delete person with" in str(e):
                raise e
            # If subscription check fails, log but continue (table might not exist)
            self.logger.warning(f"Could not check subscriptions for person {person_id}: {e}")
        ...
        try:
            ...
            await self._log_database_operation(
                ...
                before_state=before_state,
                success=success,
                error_message=None if success else "Person not found or already deleted"
            )
    ...
    async def search_people(self, search_params: Dict[str, Any], limit: int = 100, offset: int = 0, context: Optional[ErrorContext] = None) -> tuple[List[Person], int]:
        ...
        try:
            ...
            if search_params.get('first_name'):
                filter_expressions.append(Attr('firstName').contains(search_params['first_name']))

            # Search by last name (case-insensitive contains)
            if search_params.get('last_name'):
                filter_expressions.append(Attr('lastName').contains(search_params['last_name']))

            # Search by phone (contains - allows partial matches)
            if search_params.get('phone'):
                filter_expressions.append(Attr('phone').contains(search_params['phone']))

            # Filter by active status
            if search_params.get('is_active') is not None:
                filter_expressions.append(Attr('isActive').eq(search_params['is_active']))

            # Filter by email verification status
            if search_params.get('email_verified') is not None:
                filter_expressions.append(Attr('emailVerified').eq(search_params['email_verified']))
    ...
    async def get_person_by_email(self, email: str, context: Optional[ErrorContext] = None) -> Optional[Person]:
    ...
    async def update_last_login(self, person_id: str, login_time: datetime, context: Optional[ErrorContext] = None):
    ...
    async def log_security_event(self, security_event, context: Optional[ErrorContext] = None):
        ...
        try:
            ...
            if hasattr(security_event, 'to_dict'):
                ...
                item = {
                    ...
                    'timestamp': getattr(security_event, 'timestamp', datetime.utcnow()).isoformat(),
                    'success': getattr(security_event, 'success', False),
                    'eventType': (getattr(security_event, 'event_type', 'UNKNOWN').value
                ...
                if hasattr(security_event, 'severity') and security_event.severity:
                    item['severity'] = security_event.severity.value if hasattr(security_event.severity, 'value') else security_event.severity
            ...
            self.logger.info(f"Security event logged: {item.get('action', 'UNKNOWN')} - ID: {item.get('id')}")
    ...
    async def get_account_lockout(self, person_id: str, context: Optional[ErrorContext] = None) -> Optional[AccountLockout]:
        """Get account lockout information for a person with audit logging"""
        if not self.lockout_table:
            self.logger.warning("Lockout table not available, cannot get account lockout information")
            return None
        ...
        try:
            ...
            if 'Item' in response:
                ...
                lockout = AccountLockout(
                    ...
                    locked_until=datetime.fromisoformat(item['lockedUntil']) if item.get('lockedUntil') else None,
                ...
                await self._log_database_operation(
                    ...
                    additional_data={
                        ...
                        "is_locked": lockout.locked_until is not None and lockout.locked_until > datetime.utcnow()
                    }
                )
    ...
    async def save_account_lockout(self, lockout_info: AccountLockout, context: Optional[ErrorContext] = None):
        """Save account lockout information with audit logging"""
        if not self.lockout_table:
            self.logger.warning("Lockout table not available, cannot save account lockout information")
            return
        ...
        try:
            ...
            try:
                existing_lockout = await self.get_account_lockout(lockout_info.person_id, context)
            except Exception:
            ...
            await self._log_database_operation(
                ...
                additional_data={
                    ...
                    "is_locked": lockout_info.locked_until is not None and lockout_info.locked_until > datetime.utcnow(),
    ...
    async def clear_account_lockout(self, person_id: str, context: Optional[ErrorContext] = None):
        """Clear account lockout information with audit logging"""
        if not self.lockout_table:
            self.logger.warning("Lockout table not available, cannot clear account lockout information")
            return
    ...
    def create_project(self, project_data: ProjectCreate, created_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception(f"Failed to create project: {e.response['Error']['Message']}")
    ...
    def update_project(self, project_id: str, project_data: ProjectUpdate) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                return None
            raise Exception(f"Failed to update project: {e.response['Error']['Message']}")
    ...
    def delete_project(self, project_id: str) -> bool:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
                return False
            raise Exception(f"Failed to delete project: {e.response['Error']['Message']}")

    # ==================== SUBSCRIPTION METHODS ====================

    def create_subscription(self, subscription_data: SubscriptionCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception(f"Failed to create subscription: {e.response['Error']['Message']}")
    ...
    def get_all_subscriptions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"Failed to get subscriptions: {e.response['Error']['Message']}")
    ...
    def get_subscription_by_id(self, subscription_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"Failed to get subscription: {e.response['Error']['Message']}")
    ...
    def get_subscriptions_by_person(self, person_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"Failed to get person subscriptions: {e.response['Error']['Message']}")
    ...
    def get_subscriptions_by_project(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"Failed to get project subscriptions: {e.response['Error']['Message']}")

    def update_subscription(self, subscription_id: str, subscription_data: SubscriptionUpdate) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                return None
            raise Exception(f"Failed to update subscription: {e.response['Error']['Message']}")
    ...
    def delete_subscription(self, subscription_id: str) -> bool:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
                return False
            raise Exception(f"Failed to delete subscription: {e.response['Error']['Message']}")
