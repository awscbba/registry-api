...
class SecurityDashboardService:
    ...
    def __init__(self):
        ...
        self.password_reset_tokens_table = self.dynamodb.Table(
    ...
    def _get_failed_login_stats(
        ...
        try:
            ...
            response = self.audit_logs_table.scan(
                FilterExpression=(
                    Attr("eventType").eq("FAILED_LOGIN")
            ...
            return {
                ...
                "top_failed_emails": sorted(
                    email_stats.items(), key=lambda x: x[1], reverse=True
                )[:10],
                "top_failed_ips": sorted(
                    ip_stats.items(), key=lambda x: x[1], reverse=True
                )[:10],
                "recent_events": sorted(
                    failed_logins, key=lambda x: x["timestamp"], reverse=True
                )[:20],
            }
    ...
    def _get_password_reset_stats(
        ...
        try:
            ...
            return {
                ...
                "top_reset_emails": sorted(
                    email_stats.items(), key=lambda x: x[1], reverse=True
                )[:10],
                "recent_requests": sorted(
                    reset_requests, key=lambda x: x["createdAt"], reverse=True
                )[:20],
            }
    ...
    def _get_account_lockout_stats(
        self, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.people_table.scan(
                FilterExpression=Attr("failedLoginAttempts").gte(5)
                | Attr("isActive").eq(False)
            )
            ...
            lockout_events_response = self.audit_logs_table.scan(
                FilterExpression=(
                    Attr("eventType").eq("ACCOUNT_LOCKED")
                    & Attr("timestamp").between(
                        start_date.isoformat(), end_date.isoformat()
                    )
                )
            ...
            return {
                "currently_locked_count": len(
                    [acc for acc in locked_accounts if not acc.get("isActive", True)]
                ),
                "high_failed_attempts_count": len(
                    [
                        acc
                        for acc in locked_accounts
                        if acc.get("failedLoginAttempts", 0) >= 5
                    ]
                ),
                ...
                "recent_lockout_events": sorted(
                    lockout_events, key=lambda x: x["timestamp"], reverse=True
                )[:10],
            }
            ...
            return {
                ...
                "daily_lockouts": {},
                "locked_accounts": [],
                "recent_lockout_events": [],
            }

    def _get_security_event_stats(
        self, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "users_with_multiple_sessions": len(
                    [user for user, count in user_stats.items() if count > 1]
                ),
                "recent_sessions": sorted(
                    active_sessions,
                    key=lambda x: x.get("lastActivity", x.get("createdAt", "")),
                    reverse=True,
                )[:20],
            }
    ...
    def _get_user_activity_stats(
        ...
        try:
            ...
            response = self.audit_logs_table.scan(
                FilterExpression=(
                    Attr("eventType").eq("LOGIN_SUCCESS")
                    & Attr("timestamp").between(
                        start_date.isoformat(), end_date.isoformat()
                    )
                )
            ...
            for event in login_events:
                ...
                if (
                    not user_activity[user_email]["last_login"]
                    or event_time > user_activity[user_email]["last_login"]
                ):
            ...
            return {
                ...
                "most_active_users": sorted(
                    [
                        (email, data["login_count"], data["last_login"])
    ...
    def _trigger_security_alert(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            print(
                f"🚨 SECURITY ALERT: {event_data['eventType']} - {event_data['severity']}"
            )
    ...
    def get_failed_login_details(
        self, email: str = None, ip_address: str = None, hours: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            filter_expression = Attr("eventType").eq("FAILED_LOGIN") & Attr(
                "timestamp"
    ...
    def unlock_user_account(self, user_id: str, admin_email: str) -> bool:
        ...
        try:
            ...
            self.people_table.update_item(
                ...
                ExpressionAttributeValues={":zero": 0, ":active": True, ":null": None},
            )
