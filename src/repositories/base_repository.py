...
class BaseRepository(ABC, Generic[T]):
    ...
    def _log_operation(self, operation: str, duration: float, success: bool, **kwargs):
        ...
        log_data = {
            ...
            "success": success,
            "avg_response_time_ms": round((self._total_response_time / self._operation_count) * 1000, 2),
            **kwargs
        }
    ...
    def _handle_error(self, operation: str, error: Exception) -> RepositoryResult[T]:
        ...
        self.logger.error(f"Repository {operation} failed: {error_message}", 
    ...
    async def get_by_id(self, entity_id: str) -> RepositoryResult[T]:
        ...
        try:
            ...
            if 'Item' not in response:
                self._log_operation("get_by_id", duration, True, entity_id=entity_id, found=False)
                return RepositoryResult[T](success=True, data=None)
            
            entity = self._to_entity(response['Item'])
            self._log_operation("get_by_id", duration, True, entity_id=entity_id, found=True)
            
            return RepositoryResult[T](success=True, data=entity)
            
        except Exception as e:
            duration = time.time() - start_time
            self._log_operation("get_by_id", duration, False, entity_id=entity_id, error=str(e))
    ...
    async def update(self, entity: T) -> RepositoryResult[T]:
        ...
        try:
            ...
            for key, value in item.items():
                if key not in primary_key and key not in ['created_at', 'updated_at', 'version']:
            ...
            duration = time.time() - start_time
            self._log_operation("update", duration, True, entity_id=primary_key.get('id'))
    ...
    async def delete(self, entity_id: str) -> RepositoryResult[bool]:
        ...
        try:
            ...
            duration = time.time() - start_time
            self._log_operation("delete", duration, False, entity_id=entity_id, error=str(e))
            return self._handle_error("delete", e)

    async def list_all(self, options: Optional[QueryOptions] = None) -> RepositoryResult[List[T]]:
        ...
        try:
            ...
            if options:
                if options.filters:
                    scan_kwargs['FilterExpression'] = self._build_filter_expression(options.filters)
    ...
    async def query_by_index(self, index_name: str, key_condition: Dict[str, Any], 
                           options: Optional[QueryOptions] = None) -> RepositoryResult[List[T]]:
        ...
        try:
            ...
            if options:
                if options.filters:
                    query_kwargs['FilterExpression'] = self._build_filter_expression(options.filters)
            ...
            duration = time.time() - start_time
            self._log_operation("query_by_index", duration, True, 
                              index=index_name, count=len(entities))
            ...
            duration = time.time() - start_time
            self._log_operation("query_by_index", duration, False, 
    ...
    async def batch_get(self, entity_ids: List[str]) -> RepositoryResult[List[T]]:
        ...
        try:
            ...
            duration = time.time() - start_time
            self._log_operation("batch_get", duration, True, 
                              requested=len(entity_ids), found=len(all_entities))
            
            return RepositoryResult[List[T]](success=True, data=all_entities)
            
        except Exception as e:
            duration = time.time() - start_time
            self._log_operation("batch_get", duration, False, 
                              requested=len(entity_ids), error=str(e))
            return self._handle_error("batch_get", e)

    async def count(self, options: Optional[QueryOptions] = None) -> RepositoryResult[int]:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_response_time_ms": round((self._total_response_time / max(self._operation_count, 1)) * 1000, 2),
