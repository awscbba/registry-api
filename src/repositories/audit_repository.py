...
class AuditRepository(BaseRepository[AuditLog]):
    ...
    async def delete_old_audit_logs(
        ...
        try:
            ...
            return RepositoryResult(
                success=False,
                error=f"Failed to delete old audit logs: {str(e)}"
            )
    ...
    async def get_compliance_report(
        ...
        try:
            ...
            report = {
                ...
                "user_filter": user_id,
                "event_breakdown": {
                    "create_operations": len([log for log in audit_logs if log.action == "CREATE"]),
                    "update_operations": len([log for log in audit_logs if log.action == "UPDATE"]),
                    "delete_operations": len([log for log in audit_logs if log.action == "DELETE"]),
                    "read_operations": len([log for log in audit_logs if log.action == "READ"])
                },
                "resource_access": {},
                "user_activity": {},
                "security_events": []
            ...
            for log in audit_logs:
                ...
                report["user_activity"][log.user_id]["actions_by_type"][action_type] = \
                    report["user_activity"][log.user_id]["actions_by_type"].get(action_type, 0) + 1
