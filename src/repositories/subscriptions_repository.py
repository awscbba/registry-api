...
class SubscriptionsRepository(BaseRepository[Subscription]):
    ...
    async def get_by_id(self, subscription_id: str) -> Optional[Subscription]:
        ...
        subscription_data = await db.get_item(self.table_name, {"id": subscription_id})
    ...
    async def get_by_person_and_project(
        ...
        all_subscriptions = await db.scan_table(self.table_name)
    ...
    async def delete(self, subscription_id: str) -> bool:
        ...
        return await db.delete_item(self.table_name, {"id": subscription_id})

    async def list_all(self, limit: Optional[int] = None) -> List[Subscription]:
        """List all subscriptions with optional limit."""
        subscriptions_data = await db.scan_table(self.table_name, limit=limit)
        return [
