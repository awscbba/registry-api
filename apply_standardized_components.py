...
"""
                    ...
                    handler_name = file_path.stem.replace('_handler', '').replace('_', '_')
                    content = re.sub(
...
def main():
    ...
    if updated_files:
        ...
        print(f"  5. Commit changes: git add . && git commit -m 'feat: apply standardized components to all handlers'")
    else:
