...
class ProductionRiskAnalyzer:
                ...
                if 'db_service.' in line and 'await' not in line and '=' in line:
                    ...
                    if method_match:
                        ...
                        async_methods = [
                            ...
                            'get_subscription_by_id', 'create_subscription', 'update_subscription', 'delete_subscription',
                            'get_subscriptions_by_person', 'get_subscriptions_by_project',
                            'get_all_projects', 'get_all_subscriptions'
                        ]
    ...
    def generate_report(self, results: Dict[str, List[Dict]]) -> str:
        ...
        high_risk = sum(1 for issues in results.values() for issue in issues if issue.get('risk') == 'HIGH')
        medium_risk = sum(1 for issues in results.values() for issue in issues if issue.get('risk') == 'MEDIUM')
        low_risk = sum(1 for issues in results.values() for issue in issues if issue.get('risk') == 'LOW')
        ...
        for category, issues in results.items():
            ...
            for issue in issues:
                ...
                report.append(f"### {risk_emoji} {issue['type']} - {issue['risk']} Risk")
