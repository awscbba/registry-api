...
async def setup_test_admin():
    ...
    try:
        ...
        if existing_user:
            print(
                f"✅ Admin user exists: {existing_user.first_name} {existing_user.last_name}"
            )

            # Check if user has password set
            if (
                not hasattr(existing_user, "password_hash")
...
def test_admin_login():
    ...
    if response.status_code == 200:
        ...
        if admin_response.status_code == 200:
            ...
            print(
                f"   Admin user: {admin_data.get('admin_user', {}).get('firstName', 'Unknown')}"
            )
        else:
