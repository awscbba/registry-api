...
async def debug_person_update():
    ...
    try:
        ...
        if existing_person:
            ...
            print(
                f"   Person fields: {[attr for attr in dir(existing_person) if not attr.startswith('_')]}"
            )
        else:
    ...
    try:
        ...
        update_data = {"firstName": "Updated Name", "isAdmin": False}
    ...
    try:
        ...
        if updated_person:
            ...
            print(
                f"   Updated name: {updated_person.first_name} {updated_person.last_name}"
            )
            ...
            try:
                print(
                    f"   is_active: {getattr(updated_person, 'is_active', 'NOT_SET')}"
                )
                print(
                    f"   require_password_change: {getattr(updated_person, 'require_password_change', 'NOT_SET')}"
                )
                print(
                    f"   failed_login_attempts: {getattr(updated_person, 'failed_login_attempts', 'NOT_SET')}"
                )
                print(
                    f"   last_login_at: {getattr(updated_person, 'last_login_at', 'NOT_SET')}"
                )

                if updated_person.address:
                    print(
                        f"   address.postal_code: {getattr(updated_person.address, 'postal_code', 'NOT_SET')}"
                    )
                    print(
                        f"   address.country: {getattr(updated_person.address, 'country', 'NOT_SET')}"
                    )
