...
def analyze_systemic_issues():
                ...
                try:
                    ...
                    for i, line in enumerate(lines, 1):
                        if re.search(isoformat_pattern, line):
                            ...
                            if 'hasattr' not in context and 'if.*isoformat' not in context:
    ...
    print(f"   Found {len(issues['unsafe_isoformat'])} potentially unsafe .isoformat() calls")
    ...
    print(f"   Found {len(issues['unsafe_fromisoformat'])} potentially unsafe fromisoformat() calls")
            ...
            if file.endswith('.py'):
                ...
                try:
                    ...
                    if 'def update_' in content:
                        methods = re.findall(r'def (update_\w+).*?(?=def|\Z)', content, re.DOTALL)
    ...
    print(f"   Found {len(issues['missing_error_handling'])} methods with missing error handling")
        ...
        for file in files:
            if file.endswith('.py'):
                ...
                try:
                    ...
                    field_matches = re.findall(r'["\']([a-zA-Z_][a-zA-Z0-9_]*)["\']|\.([a-zA-Z_][a-zA-Z0-9_]*)', content)
            ...
            if issue_list:
                ...
                if len(issue_list) <= 5:
                    for issue in issue_list[:3]:
                        print(f"      - {issue.get('file', 'Unknown')}: {issue.get('code', issue.get('method', 'Unknown'))}")
